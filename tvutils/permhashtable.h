 ...
 * @returns             The entry where the value was saved, NULL if the key was already set with a different value.
 */
static inline int8_t tvu_permhashtable_set_s_i8(tvu_permhashtable_t * restrict table, utf8_t const * restrict key, int8_t value)
{
    tvu_int                     key_size   = strlen(key);
    tvu_hash_t                  hash       = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry     = tvu_permhashtable_set(table, hash, (uint8_t const *)key, key_size, (uint8_t const *)&value, sizeof (int8_t));

    return entry ? *(int8_t *)entry->value : 0;
}


static inline int8_t tvu_permhashtable_get_s_i8(tvu_permhashtable_t const * restrict table, utf8_t const * restrict key)
{
    tvu_int                     key_size = strlen(key);
    tvu_hash_t                  hash     = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry   = tvu_permhashtable_get(table, hash, (uint8_t const *)key, key_size);
 ...
 * @returns             The entry where the value was saved, NULL if the key was already set with a different value.
 */
static inline uint8_t tvu_permhashtable_set_s_u8(tvu_permhashtable_t * restrict table, utf8_t const * restrict key, uint8_t value)
{
    tvu_int                     key_size   = strlen(key);
    tvu_hash_t                  hash       = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry     = tvu_permhashtable_set(table, hash, (uint8_t const *)key, key_size, (uint8_t const *)&value, sizeof (uint8_t));

    return entry ? *(uint8_t *)entry->value : 0;
}


static inline uint8_t tvu_permhashtable_get_s_u8(tvu_permhashtable_t const * restrict table, utf8_t const * restrict key)
{
    tvu_int                     key_size = strlen(key);
    tvu_hash_t                  hash     = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry   = tvu_permhashtable_get(table, hash, (uint8_t const *)key, key_size);
 ...
 * @returns             The entry where the value was saved, NULL if the key was already set with a different value.
 */
static inline int16_t tvu_permhashtable_set_s_i16(tvu_permhashtable_t * restrict table, utf8_t const * restrict key, int16_t value)
{
    tvu_int                     key_size   = strlen(key);
    tvu_hash_t                  hash       = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry     = tvu_permhashtable_set(table, hash, (uint8_t const *)key, key_size, (uint8_t const *)&value, sizeof (int16_t));

    return entry ? *(int16_t *)entry->value : 0;
}


static inline int16_t tvu_permhashtable_get_s_i16(tvu_permhashtable_t const * restrict table, utf8_t const * restrict key)
{
    tvu_int                     key_size = strlen(key);
    tvu_hash_t                  hash     = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry   = tvu_permhashtable_get(table, hash, (uint8_t const *)key, key_size);
 ...
 * @returns             The entry where the value was saved, NULL if the key was already set with a different value.
 */
static inline uint16_t tvu_permhashtable_set_s_u16(tvu_permhashtable_t * restrict table, utf8_t const * restrict key, uint16_t value)
{
    tvu_int                     key_size   = strlen(key);
    tvu_hash_t                  hash       = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry     = tvu_permhashtable_set(table, hash, (uint8_t const *)key, key_size, (uint8_t const *)&value, sizeof (uint16_t));

    return entry ? *(uint16_t *)entry->value : 0;
}


static inline uint16_t tvu_permhashtable_get_s_u16(tvu_permhashtable_t const * restrict table, utf8_t const * restrict key)
{
    tvu_int                     key_size = strlen(key);
    tvu_hash_t                  hash     = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry   = tvu_permhashtable_get(table, hash, (uint8_t const *)key, key_size);
 ...
 * @returns             The entry where the value was saved, NULL if the key was already set with a different value.
 */
static inline int32_t tvu_permhashtable_set_s_i32(tvu_permhashtable_t * restrict table, utf8_t const * restrict key, int32_t value)
{
    tvu_int                     key_size   = strlen(key);
    tvu_hash_t                  hash       = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry     = tvu_permhashtable_set(table, hash, (uint8_t const *)key, key_size, (uint8_t const *)&value, sizeof (int32_t));

    return entry ? *(int32_t *)entry->value : 0;
}


static inline int32_t tvu_permhashtable_get_s_i32(tvu_permhashtable_t const * restrict table, utf8_t const * restrict key)
{
    tvu_int                     key_size = strlen(key);
    tvu_hash_t                  hash     = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry   = tvu_permhashtable_get(table, hash, (uint8_t const *)key, key_size);
 ...
 * @returns             The entry where the value was saved, NULL if the key was already set with a different value.
 */
static inline uint32_t tvu_permhashtable_set_s_u32(tvu_permhashtable_t * restrict table, utf8_t const * restrict key, uint32_t value)
{
    tvu_int                     key_size   = strlen(key);
    tvu_hash_t                  hash       = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry     = tvu_permhashtable_set(table, hash, (uint8_t const *)key, key_size, (uint8_t const *)&value, sizeof (uint32_t));

    return entry ? *(uint32_t *)entry->value : 0;
}


static inline uint32_t tvu_permhashtable_get_s_u32(tvu_permhashtable_t const * restrict table, utf8_t const * restrict key)
{
    tvu_int                     key_size = strlen(key);
    tvu_hash_t                  hash     = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry   = tvu_permhashtable_get(table, hash, (uint8_t const *)key, key_size);
 ...
 * @returns             The entry where the value was saved, NULL if the key was already set with a different value.
 */
static inline int64_t tvu_permhashtable_set_s_i64(tvu_permhashtable_t * restrict table, utf8_t const * restrict key, int64_t value)
{
    tvu_int                     key_size   = strlen(key);
    tvu_hash_t                  hash       = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry     = tvu_permhashtable_set(table, hash, (uint8_t const *)key, key_size, (uint8_t const *)&value, sizeof (int64_t));

    return entry ? *(int64_t *)entry->value : 0;
}


static inline int64_t tvu_permhashtable_get_s_i64(tvu_permhashtable_t const * restrict table, utf8_t const * restrict key)
{
    tvu_int                     key_size = strlen(key);
    tvu_hash_t                  hash     = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry   = tvu_permhashtable_get(table, hash, (uint8_t const *)key, key_size);
 ...
 * @returns             The entry where the value was saved, NULL if the key was already set with a different value.
 */
static inline uint64_t tvu_permhashtable_set_s_u64(tvu_permhashtable_t * restrict table, utf8_t const * restrict key, uint64_t value)
{
    tvu_int                     key_size   = strlen(key);
    tvu_hash_t                  hash       = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry     = tvu_permhashtable_set(table, hash, (uint8_t const *)key, key_size, (uint8_t const *)&value, sizeof (uint64_t));

    return entry ? *(uint64_t *)entry->value : 0;
}


static inline uint64_t tvu_permhashtable_get_s_u64(tvu_permhashtable_t const * restrict table, utf8_t const * restrict key)
{
    tvu_int                     key_size = strlen(key);
    tvu_hash_t                  hash     = tvu_hash((uint8_t const *)key, key_size);
    tvu_permhashtable_entry_t   *entry   = tvu_permhashtable_get(table, hash, (uint8_t const *)key, key_size);
